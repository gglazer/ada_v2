  ...
  const loadFrontend = (retries = 3) => {
    ...
    loadPromise
      ...
      .catch((err) => {
        ...
        if (retries > 0) {
          console.log(
            `Retrying in 1 second... (${retries} retries left)`
          );
          setTimeout(() => loadFrontend(retries - 1), 1000);
        } else {
          console.error(
            'Failed to load frontend after all retries. Keeping window open.'
          );
          windowWasShown = true;
...
app.whenReady().then(() => {
  ...
  checkBackendPort(8000).then((isTaken) => {
    if (isTaken) {
      console.log(
        'Port 8000 is taken. Assuming backend is already running manually.'
      );
...
function waitForBackend() {
  return new Promise((resolve) => {
    const check = () => {
      ...
      http
        .get('http://127.0.0.1:8000/status', (res) => {
...
app.on('window-all-closed', () => {
  ...
  if (process.platform !== 'darwin' && windowWasShown) {
    ...
    console.log(
      'Window was never shown - keeping app alive to allow retries'
    );
  }
});
